...
class SchemaUtilsSuite extends PlanTest with SharedSparkSession {
  test("can update metadata") {
    ...
    val new_schema = StructType(List(StructField("a", IntegerType, nullable = false, a_metadata),
    ...
    val expected = Project(Stream(
      ...
      Alias(Literal(2), "b")(exprId = id2)),
      OneRowRelation())
    assert(expected.output.toStructType.json == result.queryExecution.analyzed.output.toStructType.json)
    assert(result.queryExecution.analyzed.output.toStructType.fields(0).metadata.contains("test"))
    assert(result.queryExecution.analyzed.output.toStructType.fields(0).metadata.getLong("test") == 42)
    val missing_metadata = Project(Stream(
      Alias(Literal(1), "a")(exprId = id1),
      Alias(Literal(2), "b")(exprId = id2)),
      OneRowRelation())
    assert(missing_metadata.output.toStructType.json != result.queryExecution.analyzed.output.toStructType.json)
  }
